...
--
-- Host: 127.0.0.1
-- Generation Time: Mar 18, 2023 at 05:35 AM
...
(3, 4, 231000, 10, 1, '2023-03-17 07:15:12', '2023-03-17 07:15:12'),
...
--
DELIMITER $$
CREATE TRIGGER `delete_transaction_details` AFTER DELETE ON `transactions` FOR EACH ROW DELETE FROM transaction_details WHERE id_transactions = OLD.id
$$
DELIMITER ;
...
--
DELIMITER $$
CREATE TRIGGER `update_total_price_transactions` AFTER DELETE ON `transaction_details` FOR EACH ROW UPDATE transactions SET total_number_of_products = total_number_of_products - OLD.number_of_products WHERE id = OLD.id_transactions
$$
DELIMITER ;
